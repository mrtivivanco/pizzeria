...
char* dls(int *size, order *orders) {
    ...
    // Encontrar el día con menos ingresos
    int minIndex = 0;
    for (int i = 1; i < unique; i++) {
        if (totalPerDay[i] < totalPerDay[minIndex]) {
            minIndex = i;
        }
    }
    ...
    sprintf(result, "Día con menos dinero: %s con un total de %.2f", orderDates[minIndex], totalPerDay[minIndex]);
    return result;
}
